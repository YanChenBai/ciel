...
export class LiveMedia {
  ...
  async open(roomId: number): Promise<void> {
    ...
    this.samples = 0;
    child.stdout?.on('data', chunk => this.audio(Buffer.from(chunk)));
    const images = child.stdio[3] as Readable | null;
    images?.on('data', chunk => this.images(Buffer.from(chunk)));
    ...
    child.on('error', error => console.error('[watch-blive:ffmpeg]', error));
  }
  ...
  private publish(signal: AnySignal): void {
    if (!this.emit) return;
    const pending = this.emit(signal);
...
function ffmpegArgs(roomId: number, url: string): string[] {
  return [
    '-hide_banner',
    '-loglevel',
    'error',
