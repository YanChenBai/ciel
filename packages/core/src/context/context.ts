...
export class Context {
  ...
  private buildSystem(source: ContextBuildInput): string {
    ...
    return [
      ...
      记忆具有来源作用域。当前场景记忆只描述该场景；其他场景的记忆只能作为可迁移经验，
      不得当作当前人物或场景已经发生过的事实。

      当前上下文不足时，使用 memory_recall 按语义搜索历史经历。默认搜索当前场景；
      只有需要借鉴其他场景经验时才使用 all，并保留召回结果中的来源信息。

      发现新的稳定事实、偏好或反复验证的经验时，使用 memory_update：
      - 当前人物或场景专属的事实写入 current；
      - 跨场景稳定成立的事实、偏好或通用经验写入 global；
      - 一次性的临时事件由最近经历保存，不要提升为长期记忆；
      - 每次只提交目标作用域内精炼、去重后的完整内容，不得删除仍然有效的旧记忆。
      `),
    ]
